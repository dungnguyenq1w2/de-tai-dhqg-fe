...
export default function NationalConferencePaperFields({ nationalConferencePaperFields, control, errors }) {
	return (
		<>
			<label className='fw-semibold form-label'>2.4. Đăng trên kỷ yếu Hội nghị trong nước</label>

			{nationalConferencePaperFields.fields?.map((item, index) => (
				<div key={item.id} className='position-relative'>
					...
					<OverlayTrigger key='top' placement='top' overlay={<Tooltip id='tooltip-top'>Xóa bài báo</Tooltip>}>
						<i
							type='button'
							onClick={() => nationalConferencePaperFields.remove(index)}
			...
			<div className='d-flex justify-content-center'>
				...
				>
					<i
						type='button'
						onClick={() => {
							nationalConferencePaperFields.append()
						}}
